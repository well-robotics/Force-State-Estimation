...
namespace orien_ekf
{
    ...
    orien_ekf::orien_ekf(const std::string &name) : Node(name)
    {
        ...
        this->declare_parameter("using_vo", 0);
        ...
        using_vo = this->get_parameter("using_vo").as_int();
        ...
        gt_quaternion_ << q_init[0], q_init[1], q_init[2], q_init[3];
        ...
        low_state_sub = create_subscription<unitree_go::msg::LowState>("/lowstate",
                                                                       10,
                                                                       std::bind(&orien_ekf::low_state_callback, this, std::placeholders::_1));
    ...
    void orien_ekf::vo_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        vo_pose_quaternion_(1) = vo_b.x();
        vo_pose_quaternion_(2) = vo_b.y();
        vo_pose_quaternion_(3) = vo_b.z();
        vo_pose_quaternion_(0) = vo_b.w();
        vo_new_ = true;
        init_vo = 1;
    }
    ...
    void orien_ekf::mocap_callback(const optitrack_broadcast::msg::Mocap::SharedPtr msg)
    {
        ...
        gt_quaternion_(1) = msg->quaternion[1];
        gt_quaternion_(2) = msg->quaternion[2];
        gt_quaternion_(3) = msg->quaternion[3];
        gt_quaternion_(0) = msg->quaternion[0];
    }
    ...
    void orien_ekf::imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        accel_b_ = R_bi * accel_b_;
        angular_vel_b_ = R_bi * angular_vel_b_;

        init_imu = 1;
    }

    void orien_ekf::low_state_callback(const unitree_go::msg::LowState::SharedPtr msg)
    {

        imu_time_ = static_cast<double>(rclcpp::Clock().now().nanoseconds()) / 1e9 - time_init_; // Correctly obtaining the timestamp in seconds as a double

        accel_b_(0) = msg->imu_state.accelerometer[0];
        accel_b_(1) = msg->imu_state.accelerometer[1];
        accel_b_(2) = msg->imu_state.accelerometer[2];

        angular_vel_b_(0) = msg->imu_state.gyroscope[0];
        angular_vel_b_(1) = msg->imu_state.gyroscope[1];
        angular_vel_b_(2) = msg->imu_state.gyroscope[2];

        gt_quaternion_(1) = msg->imu_state.quaternion[1];
        gt_quaternion_(2) = msg->imu_state.quaternion[2];
        gt_quaternion_(3) = msg->imu_state.quaternion[3];
        gt_quaternion_(0) = msg->imu_state.quaternion[0];
    ...
    void orien_ekf::timerCallback()
    {
        if (init_imu)
        {
            ...
            // Quaterniond q_est;
            // q_est.w() = quaternion_(0);
            // q_est.x() = quaternion_(1);
            // q_est.y() = quaternion_(2);
            // q_est.z() = quaternion_(3);

            // Quaterniond q_gt;
            // q_gt.w() = gt_quaternion_(0);
            // q_gt.x() = gt_quaternion_(1);
            // q_gt.y() = gt_quaternion_(2);
            // q_gt.z() = gt_quaternion_(3);

            // Quaterniond q_bi = q_gt.inverse() * q_est;
    ...
    void orien_ekf::gyro_nonlinear_correct(VectorXd &quaternion_correct, VectorXd &quaternion_pred, Vector3d &accel_readings, MatrixXd &Cov_q_pred, MatrixXd &Cov_q_correct)
    {
        ...
        MatrixXd K = Cov_q_pred * H.transpose() * (H * Cov_q_pred * H.transpose() + accel_relative_norm * std::pow(accel_relative_norm, 1) * C_accel_).inverse();
    ...
    void orien_ekf::get_measurement()
    {
        ...
        if (vo_new_ && imu_time_stack_.size() > 0 && using_vo) // log when new vo transformation was subscribed
        {
            ...
            else // this if condition should only happens at during the initalization, so give some time for the imu to store enough info
            {
                ...
                for (int i = 0; i < relative_discrete_time - 1; i++)
                {
                    ...
                    if (init_cali == 0)
                    {
                        VectorXd vo_reading_inv = VectorXd::Zero(4);
                        quat_inv(vo_reading, vo_reading_inv);
                        quat_mul(quaternion_correct_, vo_reading_inv, vo_off_);
                        init_cali = 1;
                    }
                    VectorXd vo_reading_off = VectorXd::Zero(4);
                    quat_mul(vo_reading, vo_off_, vo_reading_off);
                    ...
                    if (i == 0)
                    {
                        ...
                        vo_nonlinear_correct(quaternion_correct_, quaternion_pred_, vo_reading_off, Cov_q_pred_, Cov_q_correct_);
                    }
                    quaternion_ = quaternion_correct_;
                    Cov_q_ = Cov_q_correct_;
                }
